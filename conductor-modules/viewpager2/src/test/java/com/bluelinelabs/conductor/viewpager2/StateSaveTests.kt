...
import com.bluelinelabs.conductor.viewpager2.util.TestActivity
...
class StateSaveTests {
    ...
    init {
        val activityController = Robolectric.buildActivity(TestActivity::class.java).setup()
        val layout = FrameLayout(activityController.get())
        activityController.get().setContentView(layout)
        val router = Conductor.attachRouter(activityController.get(), FrameLayout(activityController.get()))
